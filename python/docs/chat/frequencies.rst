...
For word combinatorics, :class:`~rustling.chat.CHAT` provides the
:meth:`~rustling.chat.CHAT.word_ngrams` method:
...
:meth:`~rustling.chat.CHAT.word_ngrams` takes a single required argument ``n``
for the n-gram order.
It returns an :class:`rustling.ngram.Ngrams` object, which stores ngrams efficiently and
provides useful methods such as :meth:`~rustling.ngram.Ngrams.most_common`, :meth:`~rustling.ngram.Ngrams.to_counter`,
:meth:`~rustling.ngram.Ngrams.items`, and :meth:`~rustling.ngram.Ngrams.count`.

N-grams do not cross utterance boundaries.

To filter by participant, use :meth:`~rustling.chat.CHAT.filter`
before calling :meth:`~rustling.chat.CHAT.word_ngrams`.
...
The :meth:`~rustling.chat.CHAT.word_ngrams` method returns an :class:`rustling.ngram.Ngrams` object,
which has the :meth:`~rustling.ngram.Ngrams.most_common` method as used above.
If you need a :class:`~collections.Counter` instead,
call :meth:`~rustling.ngram.Ngrams.to_counter` on the :class:`rustling.ngram.Ngrams` object:
