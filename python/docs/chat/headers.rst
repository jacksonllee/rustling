...
:class:`~rustling.chat.CHAT` has the following methods
...
.. code-block:: python
    ...
    eve = rustling.read_chat(path, filter_files="Eve")

Ages
----

:meth:`~rustling.chat.CHAT.ages` returns the age information of the participant ``"CHI"``
-- typically, only the age of the target child is available.

:meth:`~rustling.chat.CHAT.ages` understands CHAT's age format that looks like ``1;06.00``
(= one year, six months, and zero days old)
and wraps it in a helper class :class:`~rustling.chat.Age`:
...
:class:`~rustling.chat.Age` has the method :meth:`~rustling.chat.Age.in_months`
...
:meth:`~rustling.chat.CHAT.languages` returns the language information.
...
:meth:`~rustling.chat.CHAT.participants` returns the participants (e.g., ``"CHI"``)
...
:class:`~rustling.chat.Participant`. For instance:
...
:meth:`~rustling.chat.CHAT.headers` gives a list of headers,
where each header is a :class:`~rustling.chat.Headers` object for each data file,
...
See the documentation of :class:`~rustling.chat.Headers` for more details.
