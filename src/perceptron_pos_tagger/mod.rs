...
use crate::seq_feature::{
    ...
    extract_features, validate_templates,
};
use flatbuffers;
use pyo3::prelude::*;
...
#[pyclass(name = "AveragedPerceptron", from_py_object)]
#[derive(Clone)]
...
impl PyAveragedPerceptron {
    ...
    fn new(
        ...
        if let Some(ref f) = features {
            validate_templates(f, true).map_err(pyo3::exceptions::PyValueError::new_err)?;
        }
        Ok(Self {
